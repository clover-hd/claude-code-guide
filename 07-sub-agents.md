...
.claude/
└── agents/
    ├── backend_developer.md    ← バックエンド担当
    ├── mobile_developer.md     ← モバイルアプリ担当
    ├── tech_researcher.md      ← 技術調査担当
    └── ...
```

## 実例：サブエージェント構成

実際のプロジェクトでの**9つの専門エージェント**の構成例です。

| エージェント | 役割 | できること | できないこと |
|-------------|------|-----------|-------------|
| **system_architect** | CTO/設計責任者 | 設計、API仕様、DB設計 | コードの実装 |
| **backend_developer** | API開発 | Express/Prisma実装、テスト | モバイルコードの変更 |
| **mobile_developer** | モバイル開発 | React Native実装、UI | バックエンドコードの変更 |
| **management_developer** | 管理画面開発 | React 19 + Express | モバイルコードの変更 |
| **ui_ux_designer** | UI/UXデザイン | デザインシステム管理 | コードの実装 |
| **tech_researcher** | 技術調査 | Web検索、コード調査 | ファイルの編集 |
| **qa_engineer** | 品質管理 | テスト作成、バグ報告 | プロダクトコードの編集 |
| **shared_developer** | 共通ライブラリ | Entity, Value Object | 個別サービスの実装 |
| **technical_writer** | ドキュメント管理 | 仕様書の作成・更新 | コードの実装 |
...
汎用の「backend_developer」エージェントは「一般的なバックエンド開発」の知識しか持っていません。あなたのプロジェクトの固有ルール — 技術スタック、設計パターン、ディレクトリ構成、分業の境界 — を何も知らない状態で動きます。
...
あなた: サブエージェントを作りたい。
        ...
        このプロジェクト専用のbackend_developerエージェントを作って。
