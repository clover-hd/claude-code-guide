...
エージェントを定義したら、3つの方法で呼び出せます。用途に応じて使い分けてください。

### 1. `@` メンションで指定（おすすめ）

`@` を入力するとtypeaheadが表示されます。エージェントを選択し、**続けて指示を書いてから** Enter で送信します。

```
> @.claude/agents/developer.md docs/specs/feature-crud.md の仕様に従って実装して。
```

> **注意**: エージェントのパスと指示は **1つのメッセージとして送信** してください。パスだけ先に送ると、エージェントが指示なしで動き出します。

これが最も確実な方法です。どのエージェントが動くかを明示的にコントロールできます。

### 2. 自然言語で指名

プロンプトの中でエージェント名を言及すると、Claudeが判断して委任します。

```
> developer エージェントを使って、この機能を実装して
> system_architect にこの設計をレビューさせて
```

手軽ですが、Claudeの判断次第でエージェントが呼ばれないこともあります。確実に呼びたい場合は `@` メンションを使ってください。

### 3. `--agent` フラグでセッション全体に適用

セッション全体を1つのエージェントで動かしたい場合に使います。

```bash
claude --agent .claude/agents/developer.md
```

この場合、セッション中のすべてのやり取りがそのエージェントのシステムプロンプト・ツール制限・モデル設定で動作します。
